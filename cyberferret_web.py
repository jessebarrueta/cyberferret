...
VISION_HZ = 8
FOLLOW_STEERING_GAIN = 0.9
FOLLOW_STEERING_DEADBAND = 0.08
FOLLOW_TARGET_SIZE = 0.23
FOLLOW_SLOW_SIZE = 0.17
FOLLOW_MAX_THROTTLE = 0.16
...
def update_safety():
    ...
    state.safety.control_link_ok = control_age <= CONTROL_TIMEOUT_S
...
async def event_loop():
    while True:
        # The event detector only reads state and has no hardware authority.
        with state_lock:
...
async def control_loop():
    ...
    while True:
        ...
        state.safety.emergency_stop = "space" in pressed_keys
        ...
        if autonomy_enabled:
            ...
            if not visible or target_x is None or target_size is None:
                ...
                if abs(target_x) > FOLLOW_STEERING_DEADBAND:
                    target_steering = max(
                        -1.0,
                        min(1.0, target_x * FOLLOW_STEERING_GAIN),
                    )

                if target_size >= FOLLOW_TARGET_SIZE:
                    target_throttle = 0.0
                elif target_size >= FOLLOW_SLOW_SIZE:
                    target_throttle = FOLLOW_MAX_THROTTLE * 0.5
                else:
                    target_throttle = FOLLOW_MAX_THROTTLE
            ...
            elif "s" in pressed_keys and "w" not in pressed_keys:
                target_throttle = -MAX_REVERSE

            if "a" in pressed_keys and "d" not in pressed_keys:
                target_steering = -1.0
            elif "d" in pressed_keys and "a" not in pressed_keys:
                target_steering = 1.0
        ...
        with state_lock:
            ...
            state.body.updated_at = body_now
            state.body.updated_at_epoch = body_epoch
        ...
        await asyncio.sleep(max(0.0, CONTROL_DT - spent))
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Stop propulsion before waiting for software cleanup.
        drive.stop()

        for task in (control_task, vision_task, event_task):
            if task:
                task.cancel()
...
async def websocket_endpoint(ws: WebSocket):
    ...
    pressed_keys = set()
    last_control_message = time.monotonic()
...
body {
    margin: 0; padding: 20px; background: #0d0f10; color: #e6e6e6;
...
button {
    font: inherit; padding: 8px 12px; color: #eee; background: #181b1d;
    border: 1px solid #444; border-radius: 8px; cursor: pointer;
}
button.active { border-color: #7dff9b; color: #7dff9b; }
.camera {
    position: relative; max-width: 800px; margin: 15px auto;
    background: #000; border-radius: 14px; overflow: hidden;
}
.camera img { width: 100%; display: block; }
.camera-label {
    position: absolute; top: 10px; left: 10px; padding: 5px 8px;
    background: rgba(0,0,0,.65); border-radius: 5px; font-size: 12px;
}
.panel {
    max-width: 1000px; margin: 15px auto; background: #181b1d;
    border: 1px solid #292d30; border-radius: 12px; padding: 16px;
}
.grid {
    display: grid; grid-template-columns: repeat(auto-fit,minmax(190px,1fr));
    gap: 14px;
}
.group { padding: 12px; background: #111315; border-radius: 8px; }
.group-title { font-size: 12px; opacity: .55; margin-bottom: 8px; }
...
.key {
    height: 65px; border: 1px solid #444; border-radius: 10px;
    display: flex; justify-content: center; align-items: center;
    font-size: 24px; background: #181b1d;
}
.key.active { background: #454b50; }
.space { grid-column: 1 / 4; }
.hint {
    max-width: 800px; margin: 8px auto; text-align: center;
...
<button id="manualButton" class="active">MANUAL</button>
<button id="followButton">FOLLOW MARKER</button>
</div>
</header>

<div class="camera">
<img src="/camera" alt="Cyber Ferret Vision">
<div id="cameraLabel" class="camera-label">FERRET VISION</div>
</div>
...
<div id="key-space" class="key space">STOP</div>
</div>

<div class="hint">SPACE remains emergency stop in either mode.</div>
...
function displayValue(id, value, fallback = "---") {
...
function connect() {
    ...
    ws.onmessage = event => {
        ...
        manualButton.classList.toggle("active", s.mode === "MANUAL")
        followButton.classList.toggle("active", s.mode === "FOLLOW")

        const cameraLabel = document.getElementById("cameraLabel")
        if (s.observation.visual_target_visible) {
            cameraLabel.textContent = "TARGET ACQUIRED"
            cameraLabel.className = "camera-label target"
        } else {
            cameraLabel.textContent = "FERRET VISION"
            cameraLabel.className = "camera-label"
        }

        const control = document.getElementById("controlLink")
        control.textContent = s.safety.control_link_ok
            ? "ONLINE"
            : (s.mode === "FOLLOW" ? "NOT REQUIRED" : "LOST")
        control.className =
            (s.safety.control_link_ok || s.mode === "FOLLOW") ? "good" : "bad"

        const safety = document.getElementById("safety")
        safety.textContent = s.safety.reason
        safety.className = s.safety.clear ? "good" : "bad"
    }
}
...
followButton.onclick = () => setMode("FOLLOW")
...
window.addEventListener("keydown", event => {
    ...
    if (!["w", "a", "s", "d", "space"].includes(key)) return
    event.preventDefault()

    if (followButton.classList.contains("active") && key !== "space") {
        return
    }
